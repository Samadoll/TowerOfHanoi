...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname |hanoi(4.0)(for test)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
(require racket/list)
...
(check-expect (fixed? (list '(1)
              ...
              false)
(define (fixed? g num)
  (cond [(= num RINGS) (if (member? num (first g))
                           false
                           true)]
        [else
         (local [(define (isfixed? lop num)
                   (cond [(empty? lop) false]
                         [else
                          (if (andmap (λ(x) (member? x (first lop))) (filter (λ(x) (> x (- num 1))) ALL-VALS))
                              true
                              (isfixed? (rest lop) num))]))]
           (isfixed? (rest g) num))]))
